"""
B-L Portal: relic-abundance + BBN bounds in (mX, eps) plane.
...
Mirrors get_vPDM_bounds_all.py.  Two-stage workflow:
  ...
  Stage 2 -- Secluded piece (high mass / smaller eps):
    From mX_join_max upward, build a clustered grid (Z_{B-L}-pole +
    Y -> f fbar decay thresholds + log tails) up to 10**log10mXmax.
    Root-find eps via solve_boltzmann_chempot_3phase + find_epsilon_DMRelic.

Output:
  ../output/blP/relic_joint_part_aX{alphaX:.0e}_r{r}.dat   (Stage 1 only)
  ../output/blP/relic_joint_aX{alphaX:.0e}_r{r}.dat        (Stages 1 + 2)
...
from hidden_sector_DM.HiddenSectorDM import (
    Cosmology, BLPortal, BoltzmannSolver, find_mX_DMRelic_joint,
)
...
# Z_{B-L} pole: kinetic-mixing pole at mY = m_ZBL (default 1000 GeV).
M_ZBL = 1e4

# Y -> f fbar: B-L couples to quarks (B-L=+1/3) AND leptons (B-L=-1).
DECAY_PARTNERS = (Mmu, Mtau, Mc, Mb, Mt)
...
for alphaX, r, log10mXmax in cases:
    ...
    factory = lambda mX, mY, aX=alphaX: BLPortal(
    ...
    out_part = (ROOT / "output" / "blP" /
                f"relic_joint_part_aX{alphaX:.0e}_r{r}.dat")
    out_full = (ROOT / "output" / "blP" /
