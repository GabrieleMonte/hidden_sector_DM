"""
Higgs Portal: relic-abundance + BBN bounds in (mX, eps) plane.
...
Mirrors get_vPDM_bounds_all.py.  Two-stage workflow:
  ...
  Stage 1 -- Joint piece (low mass / low eps):
    ...
    Omega_c h^2 = 0.12.  Track GoH_handoff = Gamma_phi/H at the joint
  ...
  Stage 2 -- Secluded piece (high mass / smaller eps):
    From mX_join_max upward, build a clustered grid (h-pole at
    mY ~ m_h + phi -> XX decay thresholds + log tails) up to
    10**log10mXmax.  Root-find eps via solve_boltzmann_chempot_3phase
    + find_epsilon_DMRelic.

Output:
  ../output/hP/relic_joint_part_lam{lam:.0e}_r{r}.dat   (Stage 1 only)
  ../output/hP/relic_joint_lam{lam:.0e}_r{r}.dat        (Stages 1 + 2)

Columns: mX  mY  epsX_relic  epsX_bbn  Och2  GoH_handoff  regime
where regime = 0 (joint), 1 (secluded).  epsX = sin(theta) here.
"""
...
from hidden_sector_DM.HiddenSectorDM import (
    Cosmology, HiggsPortal, BoltzmannSolver, find_mX_DMRelic_joint,
)
from hidden_sector_DM.constants import (
    Mpl, Mtau, Mc, Mb, Mt, MW, MZ, Mh, mY_relic, s0_cosmo, rhoc,
    Och2 as OCH2_TARGET,
)
...
EPS_RELIC_RANGE = (-18, -5)
EPS_BBN_RANGE   = (-20, -8)

# phi -> X Xbar decay thresholds: tau, c, b, t (fermion pairs) and
# W, Z, h (boson pairs; phi -> hh only when hh_coupling='singlet').
DECAY_PARTNERS = (Mtau, Mc, Mb, Mt, MW, MZ, Mh)
...
# ---- main loop ----------------------------------------------------
for lam, r, log10mXmax in cases:
    print(f"\n=== HiggsPortal: lam={lam}, r={r}, "
          f"log10mXmax={log10mXmax} ===", flush=True)
    factory = lambda mX, mY, l=lam: HiggsPortal(
        mX=mX, mY=mY, gX=2, gY=1, lam=l,
        include_antiparticlesX=False, include_antiparticlesY=False)
    ...
    out_part = (ROOT / "output" / "hP" /
                f"relic_joint_part_lam{lam:.0e}_r{r}.dat")
    out_full = (ROOT / "output" / "hP" /
                f"relic_joint_lam{lam:.0e}_r{r}.dat")
    ...
    if out_part.exists():
        ...
        if j_rows:
            ...
            header_part = (
                f"lam={lam:.2e}  r={r}  mX_therm={mX_therm:.6e}  "
